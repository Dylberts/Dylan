...
class Post(commands.Cog):
    ...
    async def setchannel(self, ctx, channel: discord.TextChannel):
        ...
        msg = await ctx.send(f"Announcement channel set to {channel.mention}")
    ...
    async def say(self, ctx, *, message: str):
        ...
        if not channel_id:
            msg = await ctx.send("Announcement channel not set. Use `announce setchannel` to set it.")
            await msg.delete(delay=10)
            return

        embed = discord.Embed(description=message, color=0x6EDFBA)
        confirm_msg = await ctx.send("Please confirm your announcement:", embed=embed)
        ...
        try:
            ...
            if str(reaction.emoji) == '✅':
                ...
                await ctx.send("Announcement sent!", delete_after=10)
            elif str(reaction.emoji) == '❌':
                await ctx.send("Announcement canceled. You can retype the message to edit it.", delete_after=10)
        except asyncio.TimeoutError:
            await ctx.send("You took too long to respond. Announcement canceled.", delete_after=10)
    ...
    async def forumpost(self, ctx, thread_id: int, title: str, *, content: str = ""):
        ...
        if not channel_id:
            msg = await ctx.send("Forum channel not set. Use `announce setforum` to set it.")
            await msg.delete(delay=10)
            return
